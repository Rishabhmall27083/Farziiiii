...
const E_Pravesh_Course = () => {
  ...
  const fetchCourses = () => {
    ...
    axios.get(`https://sssutms.ac.in/apitest/v2/admin/courses?session=${session}`)
      .then(response => {
  ...
  const handleSearch = async () => {
    try {
      const eligibilityResponse = await fetch("https://sssutms.ac.in/apitest/geteligibility", {
        method: "POST",
        headers: {
      ...
      // console.log(eligibilityResponse, "eligibility")
      const data = await eligibilityResponse.json();
      // console.log(data, "eligibility data")
      setEligible(data);
